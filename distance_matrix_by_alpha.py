...
OUTPUT_FOLDER = 'resultats_distance' # Dossier pour stocker les fichiers CSV
...
class RobustTanimotoIdentifier:
    """
    Classe pour calculer la similarité et la distance de Tanimoto robuste et pondérée.
    """
    ...
    def calculate_distance_matrix(self, alpha: float, epsilon: float = 1e-9):
        """
        Calcule la matrice de DISTANCE complète pour un alpha donné.
        """
        print(f"\nCalcul de la matrice de distance pour alpha = {alpha:.2f}...")
        
        w = self.get_hybrid_weights(alpha).values
        
        # Fonction de distance pour scikit-learn. C'est la logique centrale.
        def distance_function(u, v):
            ...
            # La fonction retourne directement la distance
            return 1.0 - similarity

        # Calculer la matrice de distance en utilisant notre fonction personnalisée
        distance_matrix = pairwise_distances(self.taxa_matrix.values, metric=distance_function, n_jobs=-1)
...
# --- Script principal ---
def generate_all_distance_matrices():
    """
    Script principal qui charge les données et génère un fichier de distance CSV pour chaque alpha.
    """
    try:
        ...
        for alpha in ALPHA_VALUES_TO_TEST:
            ...
            # Calculer la matrice de DISTANCE pour l'alpha courant
            distance_matrix = identifier.calculate_distance_matrix(alpha=alpha, epsilon=EPSILON)
            
            # Définir un nom de fichier clair
            output_filename = f"distance_tanimoto_alpha_{alpha:.2f}.csv"
            output_path = os.path.join(OUTPUT_FOLDER, output_filename)
            
            # Sauvegarder la matrice dans son fichier CSV
            distance_matrix.to_csv(output_path, encoding='utf-8-sig')
            
            print(f"-> Matrice de distance sauvegardée dans : '{output_path}'")
        
        print("\n--- Opération terminée ---")
        print(f"Tous les fichiers de distance ont été générés dans le dossier '{OUTPUT_FOLDER}'.")
...
if __name__ == "__main__":
    generate_all_distance_matrices()